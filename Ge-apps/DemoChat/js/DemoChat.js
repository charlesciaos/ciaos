...
var updateInterval = 3000; // how many miliseconds to wait to get new message
...
function init() 
{
  console.log("[FLOW] DemoChat.php onload");
  ...
  oMessageBox.addEventListener('keydown', handleKey);
  ...
  oPalette.addEventListener('click', getColor);

  var oBtnSendMsg = document.getElementById("btnSendMsg");
  oBtnSendMsg.addEventListener('click', sendMessage);

  var oBtnDeleteMsg = document.getElementById("btnDeleteMsg");
  oBtnDeleteMsg.addEventListener('click', deleteMessages);
...
function checkUsername()
{
  ...
  if(oUser.value == "")
  {
    oUser.value = "MIP" + Math.floor(Math.random() * 1000);
  }
  oUser.addEventListener('blur', checkUsername); 
}
...
function handleReceivingMessages() 
{
  ...
  if (xmlHttpGetMessages.readyState == 4) 
  {
    ...
    {
      // display the error message
      displayError("3:"+xmlHttpGetMessages.statusText);
    }
  }
}
...
function displayMessages(idArray, colorArray, nameArray, timeArray, messageArray)
{
  ...
  for(var i=0; i<idArray.length; i++)
  {
    ...
    var htmlMessage = "";
    htmlMessage += "<div class=\"usrname\"><b>" + name + "</b></div>";
    htmlMessage += "<div class=\"wrapper\">";
    htmlMessage += "<div class=\"bg_L\"></div>";
    //htmlMessage += "<div class=\"bg_M\" style=\"background-color:" + color + "\">"; 
    htmlMessage += "<div class=\"bg_M\">"; 
    //htmlMessage += "<b>" + name + "</b>" + " said: [" + time + "] <br/>";
    htmlMessage += message.toString();
    htmlMessage += "</div>";
    htmlMessage += "<div class=\"bg_R\"></div>";
    htmlMessage += "<div class=\"time\">" + time + "</div>";
    htmlMessage += "</div><br>"; //wrapper
...
function getColor(e)
{
  ...
  if(xmlHttpGetColor)
  {
    ...
    try
    {
      //alert("ready state"+ xmlHttpGetColor.readyState);
    ...
    catch(e)
    {
      ...
      displayError("4:"+xmlHttp.statusText);
    }
  }
}
...
function handleGettingColor() 
{
  ...
  if (xmlHttpGetColor.readyState == 4) 
  {
    ...
    if (xmlHttpGetColor.status == 200) 
    {
      ...
      {
        // display error message
        displayError("5:"+xmlHttpGetColor.statusText);
      }
    } 
    else
    {
      // display error message
      displayError("6: status =" +xmlHttpGetColor.status+"  "+xmlHttpGetColor.statusText);
    }        
  }
}
