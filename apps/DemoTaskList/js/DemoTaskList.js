    ...
    Droppables.add("trash", 
                  {
                    onDrop: function(element) 
                    {
                      ...
                      //if (deleteTask)
                      {
                        ...
                        processContent(element.id, "delTask");
                      }
                    }
                  }
    );
...
function processContent(content, action)
{
  ...
  if (xmlHttp)
  {
	...
	{
        params = "?content=" + serialize(content) + "&actions=updateList";
	}
    else if (action == "addNewTask")
    {
        ...
        if (newTask)
        {
            params = "?content=" + newTask + "&actions=addNewTask";
        }
    }
    else if (action =="delTask")
	{
        params = "?content=" + content + "&actions=delTask";
	}
    ...
    try
    {
        ...
        && cache.length>0)
        {
            ...
            xmlHttp.open("GET", ServerUrl+"drag-and-drop/drag-and-drop.php" + cacheEntry, true);
...
function handleContentKey(e) 
{
  ...
  if (e.type == "keydown") 
  {
    ...
    if(code == 13)
    {
      ...
      processContent("txtNewTask", "addNewTask");
    }
  }
  else
  {
